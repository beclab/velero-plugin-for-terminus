...
# Build
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -a -o ./_output/velero-plugin-for-terminus ./velero-plugin-for-terminus/ && \ 
  CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -a -o ./_output/cp-plugin ./hack/cp-plugin
